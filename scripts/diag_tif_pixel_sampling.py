...
GRID_SIDE = 16
OUTPUT_NAME = "tif_pixel_sampling.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sample raw GeoTIFF pixels for zero/nodata diagnostics.")
...
def inspect_tif(tif_path: Path) -> dict:
...
def scan_tifs(tif_dir: Path) -> dict:
    ...
    progress_bar = progress(len(tif_paths), "Sample tif pixels")
    try:
        for tif_path in tif_paths:
            tile_report = inspect_tif(tif_path)
...
def print_summary(report: dict, output_path: Path) -> None:
    ...
    print(
        f"tile_count={report['tile_count']} sampled_pixels={report['sampled_pixel_total']} "
