...
skip_dir_paths = {
    ...
    project_root / "embed-demo-ui",
    project_root / "wasted",
    project_root / "tests",
}

# Dockerfile里只复制 *.kml 文件，所以跳过data下的其他文件
skip_file_suffixes = {
    ".pyc",
    ".pyo",
    ".swp",
...
# 只保留特定的文件或目录（Dockerfile中COPY的内容）
keep_only_patterns = {
    "app",
    "scripts",
    "requirements.txt",
    "start_backend.sh",
    "start_build_index.sh",
    "data",  # 但data里面会被进一步过滤
}
...
for root, dirs, files in os.walk(project_root):
    ...
    for filename in files:
        ...
        if file_count % 1000 == 0:
            print(f"  已处理 {file_count} 个文件...", file=sys.stderr)
            
        src_path = root_path / filename
        rel_path = src_path.relative_to(project_root)

        # 检查文件是否应该被包含（基于Dockerfile的COPY指令）
        # Dockerfile复制: app/, data/*.kml, scripts/, requirements.txt, start_*.sh
        rel_parts = rel_path.parts
        
        # 根目录: 只保留 .txt 和 start_*.sh 文件
        if len(rel_parts) == 1:
            is_txt = filename.endswith('.txt')
            is_start_sh = filename.startswith('start_') and filename.endswith('.sh')
            if not (is_txt or is_start_sh):
                skipped_other.append((rel_path.as_posix(), "not in Dockerfile COPY"))
                continue
        # 非根目录: 只保留 app/, data/, scripts/ 这三个目录
        else:
            if rel_parts[0] not in ("app", "data", "scripts"):
                skipped_other.append((rel_path.as_posix(), "not in Dockerfile COPY"))
                continue
            
            # data目录: 只保留.kml文件
            if rel_parts[0] == "data" and src_path.suffix != ".kml":
                skipped_other.append((rel_path.as_posix(), "data directory - only .kml files included"))
                continue
...
print(f"✓ 扫描完成: {len(included)} 个文件待打包", file=sys.stderr)
